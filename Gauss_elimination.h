...
#include<string.h>
#include<conio.h>
int setcoefelimination(int start,int end,char *s)
{
    int number=0;
    int sign=-1;
    if(s[start]=='+')
    ...
    for(int j=0;j<n;j++)
    {
        ...
        for(int k=0;k<n;k++)
        {
            ...
            else
            {
                ...
                float a=(float)setcoefelimination(prevpos+1,pos,s);//code for calculating the coefficient .
                mat[j][k]=a;
            }

        }
        mat[j][n]=(float)setcoefelimination(strrchr(s,'=')-s+1,strlen(s),s);
        printf("\n");
    }
